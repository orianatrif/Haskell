...
data TestResult
  ...
  | Todo {todoTestCase :: TestCase, todoMessage :: String, todoLocation :: String, todoShowDetails :: Bool}
...
instance Show TestResult where
  ...
  show (Todo TestCase {testCaseName = name, testCasePoints = points} message location showDetails)
    | showDetails = printf "%s: %s" name (Color.yellow $ printf "(%s) TODO at %s: %s%s" (show $ noPoints points) (getFileLine location) message details)
    | otherwise = printf "%s: %s" name (Color.yellow $ printf "(%s) TODO at %s: %s" (show $ noPoints points) (getFileLine location) message)
    where
      getFileLine errStr = words errStr !! 6
